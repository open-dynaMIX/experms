...
"""
  ...
  DESCRIPTION: Parses the configuration file, check validity and store content
               to variables
...
def checkoctalperms(octalperms, chmodfd, i, debug):
    """
    Verify the permissions from the config-file.
    """
    if chmodfd == 'chmodf':
        anrede = 'chmodf'
    else:
    ...
    if octalperms != ['']:
        try:
            ...
            print >> sys.stderr, ("\033[31;1mError in section " + i + ": " +
                                  anrede + " is not an interger.\033[0m")
            return False
        if len(octalperms) != 3 and len(octalperms) != 4:
            print >> sys.stderr, ("\033[31;1mError in section " + i + ": " +
                                  anrede + " needs to be a three or four digit"
                                  " octal number.\033[0m")
            return False
        for thing in octalperms:
            if int(thing) > 7:
                print >> sys.stderr, ("\033[31;1mError in section " + i + ": "
                                      + anrede + " is not an octal number."
                                      "\033[0m")
                return False
        if debug == True:
    ...
    def __init__(self, uid, debug):
        ...
        if uid == 0:
            ...
            if os_path.isfile(home + '/.experms.conf'):
                ...
                print >> sys.stderr, ("\033[31;1mError: No configuration-file "
                                      "(~/experms.conf or ~/.experms.conf) was"
                                      " found.\033[0m")
                sys.exit(1)
        ...
        if parser.has_section('general'):
            ...
            if parser.has_option('general', log_activities):
                ...
                if self.logit == 'yes':
                    ...
                    print >> sys.stderr, ("\033[31;1mError: 'log_activities' "
            ...
            if parser.has_option('general', restore):
                ...
                if self.restore == 'yes':
                    ...
                    print >> sys.stderr, ("\033[31;1mError: 'restore' must be "
                                          "either 'yes' or 'no'\033[0m")
                    errorsoccured = True
            else:
                self.restore = restoredefault
                if debug == True:
        ...
        self.sectionname = []
        # can't use enumerate here, to make it possible to mixup the order
        # between general and directory sections in the config
        number = -1
        for i in parser.sections():
            ...
            if parser.has_option(i, dirname):
                ...
                if self.dirname[number] == '':
                    ...
                    if not os_path.isdir(self.dirname[number]):
                        print >> sys.stderr, ("\033[31;1mError in section " +
                                              i + ": 'path' " +
                                              self.dirname[number] +
                ...
                if self.owner[number] != '':
                    try:
                        ...
                        try:
                            ...
                            self.owner[number] = (getpwnam(self.owner[number])
                                                  .pw_uid)
                            usowchmoderr = False
                            self.doit[number] = 1
                            if debug == True:
                ...
                if self.group[number] != '':
                    try:
                        ...
                        try:
                            ...
                            self.group[number] = (getgrnam(self.group[number])
                                                  .gr_gid)
                            usowchmoderr = False
                            self.doit[number] = 1
                            if debug == True:
            ...
            if parser.has_option(i, excludedir):
                ...
                if exvalid == True:
                    if self.dirname[number] in self.excludedir[number]:
                        print >> sys.stderr, ("\033[31;1mError in section " + i
                                              + ": 'excludedir' " + item +
                                              " is the same like 'dirname'."
                                              "\033[0m")
                        errorsoccured = True
                    if debug == True:
                        print "'excludedir' in section '" + i + "' is valid"
                if (self.excludedir[number] == [] or
                        self.excludedir[number] == ['']):
            ...
            if parser.has_option(i, excludepattern):
                ...
                try:
                    ...
                    print >> sys.stderr, ("\033[31;1mError in section " + i +
                                          ": 'excludepattern' must be a "
                                          "regular expression.\033[0m")
                    errorsoccured = True
                else:
                    if debug == True:
                        print ("'excludepattern' in section '" + i +
                               "' is valid")
            ...
            if usowchmoderr == True:
                print >> sys.stderr, ("\033[31;1mError in section " + i +
                                      ...
                                      "experms will do exactly nothing."
                                      "\033[0m")
                errorsoccured = True
